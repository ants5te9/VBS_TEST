...
import guiSample2                                #デザイナーで作った画面をインポートする

class MyForm(Qw.QMainWindow):               #MyFormという名前でQMainWindowのサブクラス作成

    def __init__(self, parent=None):        #クラスの初期化

        super(MyForm, self).__init__(parent)            #上位クラスの初期化ルーチンを呼び出す（補足2）
        self.ui = guiSample2.Ui_Dialog()     #先ほど作ったhello.pyの中にあるクラスの
...
if __name__ == '__main__':
    ...
    wmain.show()                            #表示する
    sys.exit(app.exec_())                    #こうやって終了コードを渡して抜けるのが礼儀
